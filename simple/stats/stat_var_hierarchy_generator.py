...
def _generate_internal(
    triples: list[Triple], vertical_specs: list[VerticalSpec],
    dcid2name: dict[str, str], custom_svg_prefix: str,
    default_custom_root_svg_id: str,
    sv_hierarchy_props_blocklist: set[str]) -> "StatVarHierarchy":
  """Given a list of input triples (including stat vars), 
...
class SVPropVals:
  ...
  def gen_svg_id(self, custom_svg_prefix: str):
...
def _extract_svs(triples: list[Triple],
                 sv_hierarchy_props_blocklist: set[str]) -> list[SVPropVals]:
